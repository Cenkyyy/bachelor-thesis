...
public sealed class EntityDamageable : MonoBehaviour, IDamageable
{
    ...
    [SerializeField] private EntityDeathRewards _deathRewards;
    ...
    private void Awake()
    {
        ...
        if (_deathRewards == null)
            _deathRewards = GetComponent<EntityDeathRewards>() ?? GetComponentInParent<EntityDeathRewards>();
    }
    ...
    public void ReceiveDamage(int amount, object source = null)
    {
        ...
        if (_entityCore.RuntimeData.IsDead)
            _deathRewards?.HandleEntityDied(source);
    }
    ...
    {
        _isDead = true;
        _entityCore?.RequestState(EntityStateId.Dead);
    }
}
