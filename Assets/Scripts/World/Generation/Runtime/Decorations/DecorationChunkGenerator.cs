...
public sealed class DecorationChunkGenerator : WorldChunkGeneratorBase
{
    [Header("Dependencies")]
    ...
    private readonly Dictionary<WorldBiomeType, List<DecorationEntryData>> _entriesByBiome = new();
    private readonly DecorationModificationState _modificationState = new();
    private readonly WorldObjectInstancePool _instancePool = new();
    ...
    private IEnumerator SpawnChunkInstances(WorldRuntimeData data, Vector2Int chunkCoord, int yieldEveryOperations, YieldInstruction yieldInstruction)
    {
        ...
        for (int i = 0; i < placements.Count; i++)
        {
            ...
            if (node != null)
            {
                ...
                var tracker = instance.GetComponent<DecorationInstanceController>();
                if (tracker == null)
                    tracker = instance.AddComponent<DecorationInstanceController>();
