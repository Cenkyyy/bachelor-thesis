...
public sealed class FoliageChunkGenerator : WorldChunkGeneratorBase
{
    [Header("Dependencies")]
    ...
    private readonly Dictionary<WorldBiomeType, List<FoliageEntryData>> _entriesByBiome = new();
    ...
    private List<FoliagePlacement> GeneratePlacementsForChunk(WorldRuntimeData data, Vector2Int chunkCoord)
    {
        ...
        int baseChunkSeed = WorldSeedUtility.CombineSeed(worldGenerator.CurrentSeed, chunkCoord.x, chunkCoord.y);
        int chunkSeed = WorldSeedUtility.CombineSeed(baseChunkSeed, 7927, 0);
