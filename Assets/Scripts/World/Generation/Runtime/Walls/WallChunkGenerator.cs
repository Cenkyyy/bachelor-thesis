...
public sealed class WallChunkGenerator : WorldChunkGeneratorBase
{
    ...
    private readonly Dictionary<WorldBiomeType, BiomeWallsListData> _wallsByBiome = new();
    ...
    private readonly WorldObjectInstancePool _orePool = new();
    ...
    private void HandleRuntimeTileDepleted(WallTileMineableRuntimeData runtimeData, Player miner, WorldItemSpawner dropSpawner)
    {
        ...
        var dataTile = runtimeData.Tile;
        if (!_runtimeByTile.TryGetValue(dataTile, out var ownedRuntimeData) || ownedRuntimeData != runtimeData)
            return;
    ...
    private PlannedChunkContent BuildChunkPlan(WorldRuntimeData data, Vector2Int chunkCoord)
    {
        ...
        int chunkSeed = WorldSeedUtility.CombineSeed(worldGenerator.CurrentSeed, chunkCoord.x, chunkCoord.y);
        int seed = WorldSeedUtility.CombineSeed(chunkSeed, 99173, -99173);
